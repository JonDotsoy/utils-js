...
describe('IndexedDBStore', () => {
    ...
    beforeEach(async () => {
        store = new IndexedDBStore(indexedDB.open("test", 1));
    });
    ...
    describe('addMessage', () => {
        it('should add messages to the store', async () => {
            const message1 = new Message({ id: '1', data: 'test1' });
            const message2 = new Message({ id: '2', data: 'test2' });
        ...
        it('should handle messages with different data types', async () => {
            const message1 = new Message({ id: '1', data: { name: 'test' } });
            const message2 = new Message({ id: '2', data: [1, 2, 3] });
    ...
    describe('getMessage', () => {
        it('should retrieve message by id', async () => {
            const message = new Message({ id: 'test-id', data: 'test-data' });
    ...
    describe('claimMessage', () => {
        it('should claim the oldest available message', async () => {
            const message1 = new Message({ id: '1', data: 'first', createdAt: Date.now() - 1000 });
            const message2 = new Message({ id: '2', data: 'second', createdAt: Date.now() });
            ...
            expect(claimed?.data).toBe('first');
        });
        ...
        it('should respect acknowledgment timeout', async () => {
            const message = new Message({ id: '1', data: 'test', createdAt: Date.now() });
            await store.addMessage(message);
        ...
        it('should handle abort signal', async () => {
            ...
            await expect(store.claimMessage(5000, Date.now(), abortController.signal))
                .rejects.toThrow();
        });
    });
    ...
    describe('acknowledgeMessage', () => {
        ...
        it('should handle acknowledging non-existent message', async () => {
            await expect(store.acknowledgeMessage('non-existent')).resolves.not.toThrow();
        });
    });
    ...
    describe('deleteMessage', () => {
        ...
        it('should handle deleting non-existent message', async () => {
            await expect(store.deleteMessage('non-existent')).resolves.not.toThrow();
        });
    });
    ...
    describe('getSize', () => {
        ...
        it('should return correct count after adding messages', async () => {
            const message1 = new Message({ id: '1', data: 'test1', createdAt: Date.now() });
            const message2 = new Message({ id: '2', data: 'test2', createdAt: Date.now() });
        ...
        it('should return correct count after operations', async () => {
            const message1 = new Message({ id: '1', data: 'test1', createdAt: Date.now() });
            const message2 = new Message({ id: '2', data: 'test2', createdAt: Date.now() });
            const message3 = new Message({ id: '3', data: 'test3', createdAt: Date.now() });
    ...
    describe('concurrent operations', () => {
        it('should handle concurrent message additions', async () => {
            const promises = [];
            for (let i = 0; i < 10; i++) {
                const message = new Message({ id: `${i}`, data: `test${i}`, createdAt: Date.now() + i });
        ...
        it('should handle concurrent claim operations', async () => {
            ...
            const claimedIds = claimed.filter(msg => msg !== null).map(msg => msg!.id);
